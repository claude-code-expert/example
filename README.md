...
| 이름 | 이메일 | GitHub | 
|------|--------|--------|
| 이남희 | [villainscode@gmail.com](mailto:villainscode@gmail.com) | https://github.com/villainscode| 
| 백승현 | [pekuid@gmail.com](mailto:pekuid@gmail.com) | | 

## 서적 링크

- [Yes24](https://www.yes24.com/product/goods/186846424)
- [교보문고](https://product.kyobobook.co.kr/detail/S000219725328)
- [알라딘](https://www.aladin.co.kr/shop/wproduct.aspx?ItemId=390782453)
...
| 문서 | 설명 |
|------|------|
| [Claude Code 치트시트 (HTML)](https://claude-code-expert.github.io/example/guide/claude-code-cheatsheet-ko.html) | v2.1.91 — 키보드 단축키, 슬래시 명령어, 설정, MCP, 권한 모드 전체 정리 (검증 배지 포함 인터랙티브 버전, v2.1.91 스냅샷) |
| [`claude-code-cheatsheet-ko.md`](guide/claude-code-cheatsheet-ko.md) | 위와 동일 내용의 Markdown 버전 |
| [미공개 예정 기능 분석 (HTML)](https://claude-code-expert.github.io/example/guide/claude-code-unshipped.html) | npm 소스 유출(2026-03-31) 시점 기준 미출시 기능 분석 (인터랙티브 버전 — 이후 일부 기능 정식 출시됨) |
| [`claude-code-unshipped.md`](guide/claude-code-unshipped.md) | 위와 동일 내용의 Markdown 버전 |

## HTML 렌더링 문서

`docs/html/` 디렉토리에서 인터랙티브 뷰어용 HTML 문서를 제공합니다. GitHub Pages로도 열람할 수 있습니다.

| 문서 | 설명 |
|------|------|
| [Claude Code 치트시트 라이트 (HTML)](https://claude-code-expert.github.io/example/docs/html/claude-code-cheatsheet-ko.html) | 2026-06 검증 기준 — 단축키, 슬래시 명령어, 설정, 권한 모드에 모델·Effort 탭까지 정리한 최신 탭형 버전 |
| [Appendix 1. GitHub 활용하기 (HTML)](https://claude-code-expert.github.io/example/docs/html/appendix-github-guide.html) | GitHub 가입, Git 설치, 명령어 모음, 브랜치 관리 전략까지 다루는 부록 가이드 |
| [grep vs codesight vs LSP (HTML)](https://claude-code-expert.github.io/example/docs/html/grep-vs-codesight-vs-lsp.html) | 코드 탐색 도구별 토큰 효율 비교 분석 |
...
│   │   ├── 2.7.4-hand-off.md          # 핸드오프 가이드 (세션 간 컨텍스트 전달)
│   │   └── Handoff.md                 # 핸드오프 템플릿 및 예제
│   └── html/                          # HTML 렌더링 문서 (인터랙티브 뷰어)
│       ├── claude-code-cheatsheet-ko.html  # Claude Code 치트시트 (2026-06 최신 탭형 버전)
│       ├── appendix-github-guide.html      # Appendix 1. GitHub 활용 가이드
│       └── grep-vs-codesight-vs-lsp.html   # grep vs codesight vs LSP 토큰 효율 비교
...
│   ├── settings.json                  # 권한·훅 설정 예제 (주석 제거 후 사용)
│   ├── skill-template.md              # 스킬 작성 템플릿
│   └── README.md                      # 템플릿 파일 연관관계 가이드 & 멀티턴 방어 설계
...
│   ├── cc-best-practice_awesome-cc-subagent.html # shanraisshan vs VoltAgent 서브에이전트 비교 (HTML)
│   ├── claude-code-cheatsheet-ko.html # Claude Code 치트시트 (HTML 인터랙티브)
│   ├── claude-code-cheatsheet-ko.md   # Claude Code 치트시트 (Markdown)
│   ├── claude-code-unshipped.html     # 미공개 기능 분석 (HTML)
│   ├── claude-code-unshipped.md       # 미공개 기능 분석 (Markdown)
│   ├── context-engineering-guide.md   # 컨텍스트 엔지니어링 가이드
│   ├── docker-deployment.md           # Docker와 배포 가이드 (책 미수록 분량)
│   ├── harness-engineering-guide.md   # 하네스 엔지니어링 가이드
│   ├── prompt-engineering-guide.md    # 프롬프트 엔지니어링 가이드
│   └── README.md                      # guide 디렉토리 안내
├── cli/                               # CLI 명령어 레퍼런스
│   ├── git_cli.md                     # Git 터미널 명령어 모음
│   └── postgres_cli.md                # PostgreSQL 치트시트·로컬 세팅·서버 관리
├── postgres/                          # 데이터베이스 레퍼런스
│   └── postgres-commands.md           # PostgreSQL 종합 명령어 레퍼런스
├── images/                            # 시각 자료
│   ├── cc-2026-03-31.jpeg             # Claude Code 소스 유출 관련 이미지
│   ├── claude-code-cheatsheet.png     # Claude Code 치트시트 이미지
│   ├── lsp.png                        # LSP 유무에 따른 토큰 소비 비교
...
**기술 스택(최소 스택, 리스트에 언급한 버전 이상 사용 권장):**
...
`guide/` 디렉토리에서 AI 코딩 도구 활용을 위한 엔지니어링 가이드를 제공합니다.

| 문서 | 설명 |
|------|------|
| `prompt-engineering-guide.md` | 프롬프트 엔지니어링 — 제로샷, 퓨샷, Chain of Thought 등 프롬프팅 기법과 Tika 프로젝트 적용 예시 |
| `context-engineering-guide.md` | 컨텍스트 엔지니어링 — 프롬프트를 넘어 AI가 보는 전체 맥락을 설계하는 방법 |
| `harness-engineering-guide.md` | 하네스 엔지니어링 — 에이전트를 위한 품질 보증 구조 설계 (Hooks, Rules, 자동 검증) |
...
| `postgres_cli.md` | PostgreSQL 접속·메타명령어 치트시트, macOS 서버 관리(brew/pg_ctl), 프로젝트 로컬 세팅 |
...
- 테이블 관리 (제약조건, 인덱스), CRUD 고급 문법 (UPSERT, CTE, JOIN)
...
| `testing.md` | `**/*.test.ts`, `**/*.spec.ts`, `tests/**/*` | 테스트 작성 규칙 (AAA 패턴) |
| `database.md` | `src/models/**/*.ts`, `src/repositories/**/*.ts`, `prisma/**/*` | 데이터베이스 규칙 (모델 정의, 쿼리) |
