...
| **자동 로드** | Claude Code가 자동 인식 | Cursor·Windsurf·Codex 등은 자동 인식 / Claude Code는 미지원 (import 또는 심볼릭 링크 필요) |
| **`@` 참조** | 지원 (자동 컨텍스트 포함) | 도구에 따라 다름 |
| **호환 방법** | CLAUDE.md에 `@AGENTS.md` 임포트 추가 또는 `ln -s AGENTS.md CLAUDE.md` | - |
...
복사할 파일:
  ...
  testing.md             → .claude/rules/testing.md  (기존 .claude/rules/testing.md가 있다면 덮어쓰지 말고 병합할 것)
    ...
    "Stop": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "echo '{\"hookSpecificOutput\":{\"hookEventName\":\"Stop\",\"additionalContext\":\"작업이 완료되었습니다. 원래 요청과 결과물이 일치하는지 검증하세요.\"}}'"
          }
        ]
      }
    ]
...
> **주의**: exit 0의 일반 stdout은 Stop에서 Claude에 전달되지 않음 — `additionalContext` JSON, `decision:block` JSON, 또는 exit 2 + stderr 중 하나를 사용해야 한다.
    ...
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "f=$(jq -r '.tool_input.file_path // \"\"'); echo \"$f\" | grep -qE '\\.(ts|tsx)$' && npx tsc --noEmit --pretty 2>&1 | head -20 || true"
          }
        ]
      }
    ]
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "cmd=$(jq -r '.tool_input.command // \"\"'); echo \"$cmd\" | grep -qE 'rm -rf|DROP TABLE|git push --force' && { echo 'BLOCKED: 위험한 명령어가 감지되었습니다.' >&2; exit 2; } || exit 0"
          }
        ]
      }
    ]
...
|-----------|------|
| `0` | 정상 진행 |
| `1` (기타 0/2 외) | 비차단 오류 — stderr는 사용자에게만 표시, 실행 계속 (Claude에 미전달) |
| `2` | **실행 차단** — 도구 호출이 취소됨, stderr가 Claude에게 피드백으로 전달됨 |
    ...
    "SubagentStop": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "echo '{\"hookSpecificOutput\":{\"hookEventName\":\"SubagentStop\",\"additionalContext\":\"서브에이전트 결과를 원래 목표와 대조하세요. 벗어났으면 무시하고 직접 수행하세요.\"}}'"
          }
        ]
      }
    ]
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "cmd=$(jq -r '.tool_input.command // \"\"'); echo \"$cmd\" | grep -qE 'rm -rf|DROP TABLE|TRUNCATE|git push --force' && { echo 'BLOCKED: dangerous command detected' >&2; exit 2; } || exit 0"
          }
        ]
      }
    ],
    ...
    "Stop": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "echo '{\"hookSpecificOutput\":{\"hookEventName\":\"Stop\",\"additionalContext\":\"원래 요청과 결과물이 일치하는지 검증하세요.\"}}'"
          }
        ]
      }
    ],
    "SubagentStop": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "echo '{\"hookSpecificOutput\":{\"hookEventName\":\"SubagentStop\",\"additionalContext\":\"서브에이전트 결과를 원래 목표와 대조하세요.\"}}'"
          }
        ]
      }
    ]
...
> 이 절의 완성 템플릿은 template/CLAUDE.md 전문에 Checkpoint/Recovery/Context 섹션을 더한 것이다 — 가드레일 문구 수정 시 두 파일을 함께 갱신할 것.
